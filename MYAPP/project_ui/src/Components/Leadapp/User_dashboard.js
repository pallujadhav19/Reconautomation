...
const UserDashboard = () => {
  ...
  // Filter function to filter records based on the user's input
  const filteredRecords = records.filter((record) => {
    return (
      (accountFilter === "" || record.account_name.toLowerCase().includes(accountFilter.toLowerCase())) &&
      (firstNameFilter === "" || record.first_name.toLowerCase().includes(firstNameFilter.toLowerCase())) &&
      (lastNameFilter === "" || record.last_name.toLowerCase().includes(lastNameFilter.toLowerCase())) &&
      (locationFilter === "" || record.location.toLowerCase().includes(locationFilter.toLowerCase()))
    );
  });
  ...
  const renderTableRows = () => {
   ...
   const exportLeadsToExcel = () => {
    ...
    const leadsData = records
      .filter(record => selectedTable === "leads")
                ...
                <Box display="flex" alignItems="center" justifyContent="space-between">
                  ...
                  <Button
                    ...
                    <Button
                      ...
                      size="small"
                      color="primary"
                      sx={{ marginRight: '30px', marginLeft: '20px' }}
                      onClick={handleOpen}
                    >
                      Daily Report
                    </Button>
